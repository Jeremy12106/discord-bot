...
class DebtGroup(app_commands.Group):
    ...
    async def add(self, interaction: discord.Interaction, debtor: discord.Member, amount: float, description: str):
        ...
        try:
            await debtor.send(
                f"你被 {interaction.user.global_name} 記了一筆債務：{amount} 元，原因：{description}"
            )
        except discord.Forbidden:
            logger.warning(f"無法私訊 {debtor.global_name}")

        await interaction.response.send_message(
            f"✅ 已記錄: {debtor.global_name} 欠 {interaction.user.global_name} {amount} 元\n📌 原因: {description}"
        )
    ...
    async def list(self, interaction: discord.Interaction, member: Optional[discord.Member] = None):
        ...
        if member:
            ...
            if member_id not in data or not data[member_id]:
                await interaction.response.send_message(f"{member.global_name} 目前沒有任何債務")
                return

            embed = discord.Embed(title=f"{member.global_name} 的債務清單", color=discord.Color.blue())
            ...
            for debtor_id, debts in data.items():
                ...
                embed.add_field(name=f"{debtor.global_name} 的債務", value=debt_text, inline=False)
    ...
    async def repay(self, interaction: discord.Interaction, debt_description: str):
        ...
        debt_index = next((i for i, d in enumerate(data[debtor_id]) if self._format_debt(d, interaction) == debt_description), None)
        ...
        if creditor:
            try:
                await creditor.send(
                    f"{interaction.user.global_name} 剛還清了一筆債務：{debt_entry['amount']} 元\n📌 原因：{debt_entry['description']}"
                )
            except discord.Forbidden:
    ...
    async def remove_by_creditor(self, interaction: discord.Interaction, debt_description: str):
        ...
        if debtor:
            try:
                await debtor.send(
                    f"{interaction.user.global_name} 已刪除你的一筆債務：{entry['amount']} 元\n📌 原因：{entry['description']}"
                )
            except discord.Forbidden:
