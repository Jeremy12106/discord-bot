...
class DebtGroup(app_commands.Group):
    def __init__(self, cog: DebtCog):
        super().__init__(name="debt", description="債務管理相關指令")
        self.cog = cog

    @app_commands.command(name="add", description="新增債務")
    @app_commands.describe(debtor="誰欠了你錢?", amount="輸入金額", description="債務原因或描述")
    async def add(self, interaction: discord.Interaction, debtor: discord.Member, amount: float, description: str):
        ...
        try:
            ...
            logger.warning(f"[債] 無法私訊 {debtor.name}")

        logger.info(f"[債] 伺服器ID: {interaction.guild_id} 成功記錄了一筆債務")
    ...
    async def list(self, interaction: discord.Interaction, member: Optional[discord.Member] = None):
        ...
        if member:
            ...
            if member_id not in data or not data[member_id]:
                await interaction.response.send_message(f"{member.name} 目前沒有任何債務紀錄")
                return
    ...
    async def top(self, interaction: discord.Interaction):
        ...
        medals = ["🥇", "🥈", "🥉"]
        for i, (user_id, total) in enumerate(sorted_debtors[:5], start=1):
    ...
    @app_commands.describe(debt_description="選擇要還清的債務項目")
    async def repay(self, interaction: discord.Interaction, debt_description: str):
        data = self.cog._load_data(interaction.guild_id)
        debtor_id = str(interaction.user.id)
        if debtor_id not in data:
            await interaction.response.send_message("你目前沒有債務需要還清！", ephemeral=True)
            return
        ...
        if creditor:
            try:
                await creditor.send(
                    f"{interaction.user.name} 剛剛還清了一筆債務！\n金額：{debt_entry['amount']} 元\n📌 原因：{debt_entry['description']}"
                )
            except discord.Forbidden:
                logger.warning(f"[債] 無法私訊債主 {creditor.name}")

        logger.info(f"[債] 伺服器ID: {interaction.guild_id} 還清了一筆債務")
    ...
    @app_commands.describe(debt_description="選擇要刪除的債務項目")
    async def remove_by_creditor(self, interaction: discord.Interaction, debt_description: str):
        ...
        if debtor:
            try:
                ...
                logger.warning(f"[債] 無法私訊債務人 {debtor.name}")

        logger.info(f"[債] 伺服器ID: {interaction.guild_id} 成功刪除了一筆債務")
        await interaction.response.send_message(f"✅ 已刪除一筆 {debtor.name} 的債務：{entry['amount']} 元 - {entry['description']}")
